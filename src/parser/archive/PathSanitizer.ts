...
export class PathSanitizer {
  ...
  public static sanitize(rawPath: string, maxDepth = 16): PathSanitizationResult {
    ...
    if (/[\x00-\x1F\x7F]/.test(rawPath)) {
      return { isValid: false, sanitizedPath: "", error: "Path contains null bytes or control characters" };
    }
    ...
    if (/^[a-zA-Z]:/.test(normalized) || normalized.startsWith("/") || normalized.startsWith("//")) {
      return { isValid: false, sanitizedPath: "", error: "Absolute paths and drive letters are forbidden" };
    }
    ...
    for (const seg of rawSegments) {
      ...
      if (rawTrimmed === ".." || rawTrimmed.includes("..")) {
        return { isValid: false, sanitizedPath: "", error: "Path traversal ('..') detected in archive entry" };
      }
      ...
      if (/[<>:"|?*]/.test(trimmed)) {
        return { isValid: false, sanitizedPath: "", error: `Illegal character in path segment: "${trimmed}"` };
      }

      // Check Windows reserved device names
      const baseName = trimmed.split(".")[0]!.toUpperCase();
      if (this.WINDOWS_RESERVED_NAMES.has(baseName)) {
        return { isValid: false, sanitizedPath: "", error: `Reserved device name forbidden: "${trimmed}"` };
      }

      if (trimmed.length > 255) {
        return { isValid: false, sanitizedPath: "", error: "Filename segment exceeds maximum 255 characters" };
      }
    ...
    if (safeSegments.length > maxDepth) {
      return { isValid: false, sanitizedPath: "", error: `Path nesting depth exceeds limit of ${maxDepth}` };
    }
