...
- [x] **`manifest.json` Verification**:
  - `id`: `onenote-spatial` (Obsidian compliant: only lowercase letters and hyphens, no "obsidian" substring, does not end in "plugin")
  - `name`: `On2Od`
  - `version`: `0.2.1` (SemVer numeric compliant: x.y.z)
  - `minAppVersion`: `1.5.0`
...
- [x] **`versions.json`**:
  - `{"0.1.0": "1.5.0", "0.2.0": "1.5.0", "0.2.1": "1.5.0"}`
...
When creating a GitHub Release for tag `0.2.1`, the following 3 files must be attached directly to the release:
